...
userAPI.post('/user-login', expressAsyncHandler(async (req, res) => {
    ...
    if (userData == null) {
        ...
        if (validCredential == false) {
            ...
            let signedToken = jwt.sign({ email: userCredentials.email }, process.env.SECRET_KEY, { expiresIn: 120 })
...
userAPI.put('/product/add-to-cart', expressAsyncHandler(async (req, res) => {
    let body = req.body
...
userAPI.put('/product/remove-from-cart', expressAsyncHandler(async (req, res) => {
    let body = req.body
    let userData = await User.findOne({ email: body.email })
    if (userData == null) {
        res.status(200).send({ message: "Invalid data!" })
...
userAPI.post('/product/buy', expressAsyncHandler(async (req, res) => {
    let body = req.body
    let userData = await User.findOne({ email: body.email })
    if (userData == null) {
        res.status(200).send({ message: "Invalid data!" })
...
userAPI.delete('/delete-user/:email', expressAsyncHandler(async (req, res) => {
